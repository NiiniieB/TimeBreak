...
io.on("connection", socket => {
  ...
  socket.on("message", function(msg) {
    ...
    if (json[0].type === LOGIN) {
      for (let i = 0; i < clients.length; i++)
        if (clients[i].pseudo === json[1].pseudo) {
          io.to(socket.id).emit(
            "smessage",
            JSON.stringify([{ type: ERRORLOGIN }, { user: clients }])
          );
          return;
        }
      ...
      io.emit(
        "smessage",
        JSON.stringify([{ type: UPDATELOGIN }, { user: clients }])
      );

      // Envoi de l'historique au nouveau client -- BUG sur l'envoie l'historique.
      if (historyMessage.length > 0) {
        // Moins de Bug avec condition supplémentaire : clients.length > 2
        for (let i = 0; i < clients.length; i++) {
          if (clients[i].id !== socket.id) {
            console.log(
              "envoi de l'historique à " +
                clients[i].pseudo +
                " ,id : " +
                clients[i].id
            );
    ...
    if (json[0].type === MESSAGE) {
      console.log(
        json[1].date + " : " + json[1].sender.pseudo + " : " + json[1].text
      );
      ...
      ); // Enregistre sur le serveur l'historique des messages
